...
*** Org Capture
:PROPERTIES:
:ID:       678ceacd-e273-4fd8-aa52-5866be634962
:END:
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-org-capture.el" :mkdirp yes
...
*** Org Agenda
:PROPERTIES:
:ID:       01a578a0-61b8-4a1f-bd4c-160f49eebbd9
:END:
The agenda should classify all of my activities into one of the
following buckets:
1. Things I need to do, eventually;
2. Things I need to complete by a specific date;
3. Things I cannot (or won't) start until a specific date;
4. Things I cannot (or won't) start until an *unspecified* date;
5. Things I should do with some loose frequency.
6. Things that I have to do every X amount of time.
Each of these aligns with an Org Mode feature:
1. A plain *TODO* entry.
2. An entry with a *DEADLINE* time stamp.
3. A *WAITING* entry with a *SCHEDULED* time stamp.
4. A *WAITING* entry without a time stamp.
5. An entry with a *STYLE* property of "habit".
6. An entry with a [[https://orgmode.org/manual/Repeated-tasks.htmll][repeaters]].
An activity however is not only described by its timeline. These are
the activity types:
1. *PROJ*
   A project entry meant to hold many subtasks. Can be *FINISHED*.
2. *TODO*
   A general entry. It can be *WAITING*, *DONE* and *CANCELLED*.
3. *RECURRING*
   For recurring tasks (think, gym, every day at specific time). Can
   be *DONE* (i.e. you stopped going to the gym).
Obviously each of these "tasks" will have a different skeleton and we
automate it using =org-capture-templates=. Note that the
capture-templates *append* the =org-capture-templates= variable, they
dont set it from scratch.
The agenda needs some sort of "view" for the user to interact with
it. This is implemented in[[id:a6e5cf7b-4291-40bd-b781-8704df97bd2d][ org-super-agenda]].
References for this implementation include [fn:1],[fn:2]
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-org-agenda.el" :mkdirp yes
...
*** Org Super Agenda
:PROPERTIES:
:ID:       a6e5cf7b-4291-40bd-b781-8704df97bd2d
:END:
An agenda custom command allows you to create custom views that are
easilly triggered. More specifically =org-agenda-custom-commands= is a
list of lists, the elemements (lists) are the custom commands which
define the view and org-super-agenda is used to compartmentalize, 
make the view prettier and easilly customizable.
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-org-superagenda.el" :mkdirp yes
...
:END:
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-org-latex-preview.el" :mkdirp yes
...
**** F*
:PROPERTIES:
:ID:       0e7538d6-ad8c-4ee6-8e0a-30555c2a4584
:END:
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-fstar.el" :mkdirp yes
  (use-package fstar-mode
    :ensure t
    :custom
    (fstar-executable "/opt/fstar/bin/fstar.exe")
    (fstar-smt-executable "/usr/bin/z3")
  )
#+end_src
**** F#
:PROPERTIES:
:ID:       a4ad1a77-f937-4b3c-bf97-61004cfc9273
:END:
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-fsharp.el" :mkdirp yes
...
**** GLSL
:PROPERTIES:
:ID:       0ae82b5c-28f0-4803-b207-9919485f650f
:END:
...
* Footnotes
[fn:2] https://orgmode.org/manual/Repeated-tasks.html 
[fn:1] https://blog.aaronbieber.com/2016/09/24/an-agenda-for-life-with-org-mode.html
