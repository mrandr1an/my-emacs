...
#+begin_src emacs-lisp :tangle "init.el"
  (defcustom my-emacs-org-publications "~/Documents/Publish/Org"
    "Where org-based publications drafts live."
    :type 'directory
  )
#+end_src
...
as follows:
 - Display items in the following order:
   1. Directories
   2. Hidden Directories (If toggled)
   3. Files
   4. Hidden Files (If toggled)
 - Toggle information of files/directories in region/cursor.
 - Toggle preview of files/directories in region/cursor.
 - Directory Functionallity
   - Move
   - Copy
   - Rename
   - Mkdir
...
The =my-emacs--dired-category= function returns the category of an item
as defined in =`my-emacs-dired-order`=. Note that this counts "." and ".."
as =`hidden-files`=.
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-dired.el" :mkdirp yes
  (defun my-emacs-dired--category (filename)
    (let* ((base (file-name-nondirectory filename))
      ...
      (cond
       ...
       (t                'files))))
#+end_src
The =my-emacs-dired--get-filename-at-point= function is just a wrapper a
#+begin_src emacs-lisp :tangle "my-emacs-packages/my-emacs-dired.el"
  (require 'cl-lib)
